<?php
// Call From Search Movies Menu Link
function nt_fn_search_movies()
{
    ...
    <div class="movie_container" style="margin-top:10px;">
      <div class="tablenav top">
        <div class="alignleft actions bulkactions">
          <input type="text" name="search_movie" id="search_movie" />
          <input type="submit" disabled="disabled" style="display:none;" />
          <input type="button" name="get_movies" id="get_movies" class="button" value="Search" />
        </div>
...
jQuery(document).ready(function() {	
	jQuery("#search_movie").focus();
	
	jQuery("#search_movie").keyup(function(e) {
        jQuery("#search_movie").css("border-color","");    
    });
	...
	jQuery("#get_movies").click(function(e) {	
		...
		var mSearch = jQuery("#search_movie").val();
		if(mSearch!='')
		{
			var moviesUrl = baseUrl+'?q='+mSearch+'&page_limit=50&apikey=' + apikey;
			// send off the query
			jQuery.jsonp({
				url: moviesUrl,
		...
		{
			jQuery("#search_movie").css("border-color","red").focus();
			return false;
		}
...
function searchCallback(data) {
	...
	else
	{
		...
		{
			tbldata += '<tr class="no-items"><td colspan="6" class="colspanchange">No Movie found</td></tr>';
		}
		else
		{
